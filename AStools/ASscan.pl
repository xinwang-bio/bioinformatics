...
### 0.2beta add a cut_anti parameter for ASscan
#

my ($input, $output, $anchor_match_length, $reference, $min_maq, $intron_reads_number, $junction_reads, $keep_tmp, $cut_anti, $help);
...
$cut_anti = $cut_anti || 0.1;

system "mkdir $output";
system "mkdir $output/tmp";
open TEST,">","$output/$output.IR.log";
#my ($in,$ref,$out)=@ARGV;
...
for my $k(sort {$a cmp $b} keys %junction_site){
	...
	############ check the possible strand which strand is primary ##############
	if(($sense >= $antisense) && ($antisense/$total <= $cut_anti)){
		print OUT"$k\t\+\t$sense\t$antisense\t$total\n";
	}
	elsif(($sense < $antisense) && ($sense/$total <= $cut_anti)){
		print OUT"$k\t\-\t$sense\t$antisense\t$total\n";
	}
}
...
print OUT"\#chrom\tstrand\tdonor\taccepter\ttotal_reads\tspanned_reads\twithin_reads\tuniq_spanned\tuniq_within\ttotal_nt\tuniq_nt\n";
my %ir_return; 
my %read_infor;
...
%read_infor = %$read_infor_return_minus;
...
print OUT"\#chrom\tstrand\tdonor\taccepter\treads\n";
print OUT1"\#chrom\tstrand\tdonor\taccepter\tmuti\treads\n";
...
print OUT"\#chrom\tstrand\tdonor\taccepter\tdonor\treads\n";
#print OUT1"\#chrom\tstrand\tdonor1\taccepter\treads\tacceptor4\treads\n";
...
Options:
     ...
     -r|reference		YES  genome file (fasta format)
     -q|maq  			NO  minumum mapping quality for scaning AS, default = 0
     -a|min_anchor  	NO  minimum anchor for junction reads for junction site scan, default = 10 bp
     -c|cut_anti		NO	cutoff for the complementary strand reads, the complementary reads must less than this ratio, default = 0.1
     -n|intron_reads  	NO  minimum reads number cover the intron region, default = 10
     -j|junction_reads  NO  minimum junction coverage, default = 3
     \n"
}
