...
my $ir_return_plus = &AS_event_IR(\%plus_doner, \%plus_acceptor, $sort_bam, $strand);
...
my $ir_return_minus = &AS_event_IR(\%minus_doner, \%minus_acceptor, $sort_bam, $strand);
...
for $t(sort {$a cmp $b} keys %aa_return){
	for $p(sort {$a <=> $b} keys %{$aa_return{$t}}){
		print OUT"$t\t$p\t$aa_return{$t}{$p}[0]\t$aa_return{$t}{$p}[1]\t$aa_return{$t}{$p}[2]\n";
	}
}

for $c(sort {$a cmp $b} keys %aa_multi_return){
	for $cc(sort {$a <=> $b} keys %{$aa_multi_return{$c}}){
		print OUT1"$c\t$cc\t$aa_multi_return{$c}{$cc}[0]\t$aa_multi_return{$c}{$cc}[1]\t$aa_multi_return{$c}{$cc}[2]\t$aa_multi_return{$c}{$cc}[3]\n";
	}
}
...
for $t(sort {$a cmp $b} keys %ad_return){
	for $p(sort {$a <=> $b} keys %{$ad_return{$t}}){
		print OUT"$t\t$p\t$ad_return{$t}{$p}[0]\t$ad_return{$t}{$p}[1]\t$ad_return{$t}{$p}[2]\n";
	}
}

for $c(sort {$a cmp $b} keys %ad_multi_return){
	for $cc(sort {$a <=> $b} keys %{$ad_multi_return{$c}}){
		print OUT1"$c\t$cc\t$ad_multi_return{$c}{$cc}[0]\t$ad_multi_return{$c}{$cc}[1]\t$ad_multi_return{$c}{$cc}[2]\t$ad_multi_return{$c}{$cc}[3]\n";
	}
}
